...
plot_score_heatmap <- function(fitness_table, WTresibox, start_resi, end_resi, legend_title){
  ...
  p <-  ggplot() +
    ...
    scale_fill_gradientn(colours=c("white","yellow","orange","orange1","orange2","orangered","red"),
                         limits=c(0,0.0015),
                         values=rescale(c(0,0.0003,0.0006,0.0009,0.0012,0.0015)),
                         breaks=c(0,0.00075,0.0015),
                         labels=c('0','0.00075','0.0015'),
                         guide="colorbar",
                         na.value="grey") +
    theme_cowplot(12) +
...
df <- read_tsv('NTD_DMS_scores.tsv') %>%
...
df <- df %>%
  ...
  arrange(Pos) %>%
  mutate(parameter=Input_freq) %>% #adjust to select the parameter of interest
...
ggsave('NTD_input_freq_heatmap.png',p,width=5.5, height=7, dpi=300)
